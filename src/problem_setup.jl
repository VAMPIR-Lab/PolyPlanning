...
function get_single_sd_ids(xt,Ae,be,Ao,bo)
    Aex, bex = shift_to(Ae,be,xt)
    AA, bb, qq = gen_LP_data(Aex,bex,Ao,bo)
    m1 = length(bex)
    m2 = length(bo)
...
function setup_quick(; T = 1,
    ...
    @showprogress for (k, sd) in sds
        ...
        lag = Symbolics.gradient(-sd*λsd[1], xt) |> simplify
        ...
        Jlag = Symbolics.jacobian(lag, [xt; λsd])
        Jsd = Symbolics.jacobian([sd], xt)
    ...
    function fill_F!(F,z,x0,Ae,be,polys,λ_nom,λ_col)
        ...
        for t in 1:T
            ...
            for (e,P) in enumerate(polys)
                ...
                λte = λ_col[λte_inds]
                assignments = get_single_sd_ids(xt,Ae,be,Ao,bo)
...
function solve_quick(prob, x0, P1, P2, P3, Pe; θ0 = nothing)
    (; fill_F!, get_J_both, l, u, T, n_z, n_nom, n_col, N_polys, sides_per_poly, p1_max, p2_min) = prob
    ...
    JJ = get_J_both(θ0[1:n_z], x0, Pe.A,Pe.b, polys,rand(n_nom),rand(n_col))
